...
function encodeBase64(content) {
  return Buffer.from(JSON.stringify(content, null, 2), "utf8").toString(
    "base64"
  );
}
      ...
      try {
        const createResponse = await axios.post(
          "https://api.github.com/user/repos",
          {
            name: GITHUB_REPO,
            description:
              "Sauvegarde des données NakamaBot - Créé automatiquement",
            private: true,
            auto_init: true,
          },
          {
...
async function saveDataToGitHub() {
  ...
  try {
    log.debug(
      `💾 Tentative de sauvegarde sur GitHub: ${GITHUB_USERNAME}/${GITHUB_REPO}`
    );
    ...
    const dataToSave = {
      ...
      userNames: Object.fromEntries(userNames), // Ajout des noms
      ...
      version: "5.1 Amicale + Vision + GitHub + Clans + Rank",
      totalUsers: userList.size,
      totalConversations: userMemory.size,
      totalImages: userLastImage.size,
      totalNames: userNames.size, // Ajout statistique
      totalClans: commandContext.clanData
        ? Object.keys(commandContext.clanData.clans || {}).length
        : 0,
      totalUsersWithExp: rankCommand
        ? Object.keys(rankCommand.getExpData()).length
        : 0,
      bot: "NakamaBot",
      creator: "Durand",
    };
    ...
    for (let attempt = 1; attempt <= maxRetries && !success; attempt++) {
      try {
        ...
        if (response.status === 200 || response.status === 201) {
          const clanCount = commandContext.clanData
            ? Object.keys(commandContext.clanData.clans || {}).length
            : 0;
          const expDataCount = rankCommand
            ? Object.keys(rankCommand.getExpData()).length
            : 0;
          log.info(
            `💾 Données sauvegardées sur GitHub (${userList.size} users, ${userMemory.size} convs, ${userLastImage.size} imgs, ${clanCount} clans, ${expDataCount} exp)`
          );
          success = true;
        } else {
        ...
        if (retryError.response?.status === 409 && attempt < maxRetries) {
          log.warning(
            `⚠️ Conflit SHA détecté (409), tentative ${attempt}/${maxRetries}, retry dans 1s...`
          );
          await sleep(1000);
          continue;
    ...
    if (error.response?.status === 404) {
      ...
      log.warning(
        "⚠️ Conflit SHA persistant - sauvegarde ignorée pour éviter les blocages"
      );
    } else {
...
async function loadDataFromGitHub() {
  if (!GITHUB_TOKEN || !GITHUB_USERNAME) {
    log.warning(
      "⚠️ Configuration GitHub manquante, utilisation du stockage temporaire uniquement"
    );
    return;
  }
  ...
  try {
    log.info(
      `🔍 Tentative de chargement depuis GitHub: ${GITHUB_USERNAME}/${GITHUB_REPO}`
    );
    ...
    if (response.status === 200 && response.data.content) {
      ...
      if (data.userList && Array.isArray(data.userList)) {
        ...
        log.info(
          `✅ ${data.userList.length} utilisateurs chargés depuis GitHub`
        );
      }
      ...
      if (data.userMemory && typeof data.userMemory === "object") {
        ...
        log.info(
          `✅ ${
            Object.keys(data.userMemory).length
          } conversations chargées depuis GitHub`
        );
      }
      ...
      if (data.userLastImage && typeof data.userLastImage === "object") {
        ...
        log.info(
          `✅ ${
            Object.keys(data.userLastImage).length
          } images chargées depuis GitHub`
        );
      }

      if (data.userNames && typeof data.userNames === "object") {
        Object.entries(data.userNames).forEach(([userId, name]) => {
          userNames.set(userId, name);
        });
        log.info(
          `✅ ${
            Object.keys(data.userNames).length
          } noms d'utilisateurs chargés depuis GitHub`
        );
      }
      ...
      if (data.userExp && typeof data.userExp === "object" && rankCommand) {
        ...
        log.info(
          `✅ ${
            Object.keys(data.userExp).length
          } données d'expérience chargées depuis GitHub`
        );
      }
      ...
      if (data.commandData && typeof data.commandData === "object") {
        ...
        log.info(
          `✅ ${
            Object.keys(data.commandData).length
          } données de commandes chargées depuis GitHub`
        );
      }
    ...
    if (error.response?.status === 404) {
      log.warning(
        "📁 Aucune sauvegarde trouvée sur GitHub - Première utilisation"
      );
      ...
      log.error(
        "❌ Accès refusé GitHub (403) - Vérifiez les permissions de votre token"
      );
    } else {
...
function addToMemory(userId, msgType, content) {
  ...
  // Vérifier les doublons avec seuil de similarité et fenêtre temporelle
  if (memory.length > 0) {
    const lastMessage = memory[memory.length - 1];
    const now = new Date();
    const lastTimestamp = new Date(lastMessage.timestamp);
    const timeDiff = (now - lastTimestamp) / 1000; // Différence en secondes

    if (
      lastMessage.type === msgType &&
      lastMessage.content === content &&
      timeDiff < 60
    ) {
      log.debug(
        `🔄 Doublon exact évité pour ${userId}: ${msgType.substring(0, 50)}...`
      );
      return;
    }
    ...
    if (
      ...
      if (similarity > 0.85) {
        log.debug(
          `🔄 Doublon assistant évité (similarité: ${Math.round(
            similarity * 100
          )}%) pour ${userId}`
        );
        return;
  ...
  log.debug(
    `💭 Ajouté en mémoire [${userId}]: ${msgType} ${content.substring(
      0,
      50
    )}...`
  );
...
async function sendMessage(recipientId, text) {
  ...
  if (
    ...
    !finalText.includes("✨ [Message tronqué avec amour]")
  ) {
    finalText =
      finalText.substring(0, 1950) + "...\n✨ [Message tronqué avec amour]";
  }
...
const commandContext = {
  ...
  getUserName: async (userId) => {
    ...
    const name = await fetchFacebookUserName(userId, commandContext);
    if (name) {
      userNames.set(userId, name);
      saveDataImmediate();
    }
...
app.get("/", (req, res) => {
  ...
  res.json({
    status:
      "🤖 NakamaBot v4.0 Amicale + Vision + GitHub + Clans + Rank Online ! 💖",
    creator: "Durand",
...
app.get("/webhook", (req, res) => {
  ...
  const userName = userNames.get(senderIdStr) || "inconnu";

  log.info(
    `📨 Message de ${senderId} (${userName}): ${messageText.substring(
      0,
      50
    )}...`
  );
    ...
    for (const entry of data.entry || []) {
      for (const event of entry.messaging || []) {
        ...
        if (event.message && !event.message.is_echo) {
                ...
                if (imageUrl) {
                  ...
                  if (rankCommand) {
                    ...
                    if (expResult.levelUp) {
                      log.info(
                        `🎉 ${senderId} a atteint le niveau ${expResult.newLevel} (image) !`
                      );
                    }
                  }
          ...
          if (messageText) {
            log.info(
              `📨 Message de ${senderId}: ${messageText.substring(0, 50)}...`
            );
            ...
            if (messageText && rankCommand) {
              ...
              if (expResult.levelUp) {
                log.info(
                  `🎉 ${senderId} a atteint le niveau ${expResult.newLevel} !`
                );
            ...
            if (response) {
              if (typeof response === "object" && response.type === "image") {
                ...
                if (sendResult.success) {
                  ...
                  const fallbackMsg =
                    "🎨 Image créée avec amour mais petite erreur d'envoi ! Réessaie ! 💕";
...
app.get("/test-github", async (req, res) => {
  try {
    ...
    if (error.response?.status === 404) {
      ...
      suggestions = [
        "Vérifiez les permissions de votre token (repo, contents)",
      ];
    }
...
app.get("/stats", (req, res) => {
  ...
  res.json({
    ...
    images_stored: userLastImage.size,
    names_stored: userNames.size, // Ajout
    clans_total: clanCount,
    users_with_exp: expDataCount,
...
app.get("/health", (req, res) => {
  ...
  const healthStatus = {
    ...
    services: {
      ...
      user_names: Boolean(PAGE_ACCESS_TOKEN), // Ajout
    },
    data: {
      users: userList.size,
      conversations: userMemory.size,
...
async function startBot() {
  log.info(
    "🚀 Démarrage NakamaBot v4.0 Amicale + Vision + GitHub + Clans + Rank"
  );
  log.info(
    "💖 Personnalité super gentille et amicale, comme une très bonne amie"
  );
  ...
  if (rankCommand) {
    ...
    log.warning(
      "⚠️ Commande rank non trouvée - Système d'expérience désactivé"
    );
  }
  ...
  log.info(
    "🎉 NakamaBot Amicale + Vision + GitHub + Clans + Rank prête à aider avec gentillesse !"
  );
  ...
  app.listen(PORT, () => {
    ...
    log.info(
      `📊 Dashboard: https://github.com/${GITHUB_USERNAME}/${GITHUB_REPO}`
    );
  });
}
